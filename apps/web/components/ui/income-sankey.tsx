...
export default function IncomeSankey({ sankeyJson }: IncomeSankeyProps) {
  ...
  const toTextAnchor = (
    ...
    const allowed: Array<NonNullable<SVGAttributes<SVGTextElement>['textAnchor']>> = [
      'start',
      'middle',
      'end',
      'inherit',
  ...
  const getSankeyNodesForReact = (
    ...
    .nodes()
    .map((node, index) => {
      const group = node as SVGGElement & { __data__?: { color?: string } };
      ...
      return {
        ...
        valueText: {
          ...
          textContent: valueText?.textContent ?? '',
        },
        titleText: {
